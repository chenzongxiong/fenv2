...
if __name__ == "__main__":
    ...
    # LOG.debug("================================================================================")
    input(colors.red("RUN script ./rnn_loss_history_collector.sh #diff_weights before run this script"))
    ...
    if argv.markov_chain:
        excel_fname = './new-dataset/rnn/diff_weights/method-sin/lstm-mc-mle-sigma-{}-__activation__-{}.xlsx'.format(sigma, __activation__)
    elif argv.diff_weights:
        # __units__ = 16
        excel_fname = './new-dataset/rnn/diff_weights/method-{}/lstm-all-sigma-{}-units-{}.xlsx'.format(method, sigma, __units__)
    else:
        excel_fname = './new-dataset/rnn/method-sin/lstm-all-sigma-{}.xlsx'.format(sigma)
    ...
    for nb_plays in nb_plays_LIST:
        ...
        # for __units__ in __units__LIST:
            ...
            if argv.diff_weights:
                prediction_fname = constants.DATASET_PATH['rnn_diff_weights_prediction'].format(method=method, activation=activation, state=state, mu=mu, sigma=sigma, units=units, nb_plays=nb_plays, points=points, input_dim=input_dim, __units__=__units__, loss='mse', ensemble=ensemble)
                loss_fname = constants.DATASET_PATH['rnn_diff_weights_loss'].format(method=method, activation=activation, state=state, mu=mu, sigma=sigma, units=units, nb_plays=nb_plays, points=points, input_dim=input_dim, __units__=__units__, loss='mse', ensemble=ensemble)
                loss_file_fname = constants.DATASET_PATH['rnn_diff_weights_loss_file'].format(method=method, activation=activation, state=state, mu=mu, sigma=sigma, units=units, nb_plays=nb_plays, points=points, input_dim=input_dim, __units__=__units__, learning_rate=lr, loss='mse', ensemble=ensemble)
            # else:
