...
if __name__ == "__main__":
    ...
    if argv.diff_weights:
        ...
        # fname = './tmp/run-gru-dima-sequence-multiple-times-{}.log'.format(seq)
        fname = './tmp/run-rnn-pavel-sequence-multiple-times-{}.log'.format(seq)

        log_fname="./new-dataset/rnn/diff_weights/method-{method}/activation-{activation}/state-{state}/input_dim-{input_dim}/mu-{mu}/sigma-{sigma}/units-{units}/nb_plays-{nb_plays}/points-{points}/units#-{__units__}/ensemble-{ensemble}/loss-mse/history-lr-{lr}.csv".format(
            method=method,
            activation=activation,
            state=0,
            input_dim=1,
