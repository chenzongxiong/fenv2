...
# input vs. output
def rnn(input_fname, units, epochs=1000, weights_fname=None, force_train=False, learning_rate=0.001):
    ...
    model.add(tf.keras.layers.SimpleRNN(int(units),
                                        input_shape=(1, 1),
                                        unroll=False,
...
if __name__ == "__main__":
    ...
    if argv.diff_weights is True:
        ...
        prediction_fname = constants.DATASET_PATH['rnn_diff_weights_prediction'].format(method=method, activation=activation, state=state, mu=mu, sigma=sigma, units=units, nb_plays=nb_plays, points=points, input_dim=input_dim, __units__=__units__, loss=loss, ensemble=ensemble)
        loss_fname = constants.DATASET_PATH['rnn_diff_weights_loss'].format(method=method, activation=activation, state=state, mu=mu, sigma=sigma, units=units, nb_plays=nb_plays, points=points, input_dim=input_dim, __units__=__units__, loss=loss, ensemble=ensemble)
        weights_fname = constants.DATASET_PATH['rnn_diff_weights_weights'].format(method=method, activation=activation, state=state, mu=mu, sigma=sigma, units=units, nb_plays=nb_plays, points=points, input_dim=input_dim, __units__=__units__, loss=loss, ensemble=ensemble)
    # else:
    ...
    if loss == 'mse':
        test_inputs, predictions, rmse, diff_tick = rnn(input_fname, units=__units__,
    ...
    LOG.debug("predict fname: {}".format(prediction_fname))
    LOG.debug('==================FINISHED=========================================================')
